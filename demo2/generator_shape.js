(function(window, An){
    ...
    Pin.onFrame = function (ctx, frameCounter) {
        ...
        if (Pin.mode == 'move' && Pin.isMoved && Pin.list[Pin.moveIndex]) {
            Pin.list[Pin.moveIndex].x = an.eventMousemove.x;
            Pin.list[Pin.moveIndex].y = an.eventMousemove.y;

            //an.Graphic.circle(an.eventMousemove.x, an.eventMousemove.y, 17, 'rgba(225,0,0,.9)', true);
            an.Graphic.linePoints(
                {x: an.eventMousemove.x-17, y: an.eventMousemove.y},
                {x: an.eventMousemove.x+17, y: an.eventMousemove.y},
                3
            );
            an.Graphic.linePoints(
                {x: an.eventMousemove.x, y: an.eventMousemove.y-17},
                {x: an.eventMousemove.x, y: an.eventMousemove.y+17},
                3
            );
        }
